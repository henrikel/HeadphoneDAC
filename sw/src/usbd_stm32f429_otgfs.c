...
#endif
						USB_OTG_GINTMSK_OTGINT | USB_OTG_GINTMSK_MMISM |
                        USB_OTG_GINTMSK_USBSUSPM | USB_OTG_GINTMSK_WUIM |
                        USB_OTG_GINTMSK_IEPINT | USB_OTG_GINTMSK_RXFLVLM |
                        USB_OTG_GINTMSK_PXFRM_IISOOXFRM | USB_OTG_GINTMSK_IISOIXFRM; // HE added
...
static bool ep_config(uint8_t ep, uint8_t eptype, uint16_t epsize) {
    ...
    if (ep & 0x80) {
        ...
        if ((eptype == USB_EPTYPE_ISOCHRONOUS) ||
...
static bool ep_activate(uint8_t ep) {
...
static void toggle_sof(uint8_t ep_num) {
	
	if(ep_num & 0x80) {
	
		USB_OTG_INEndpointTypeDef* ep = EPIN(ep_num & 0x7f);
		
		//ep->DIEPCTL = (USB_OTG_DIEPCTL_EPDIS | USB_OTG_DIEPCTL_SNAK);
		
		//if(ep->DIEPCTL & USB_OTG_DIEPCTL_EONUM_DPID)
		if((get_frame() & 1) == 0)
			ep->DIEPCTL |= USB_OTG_DIEPCTL_SODDFRM;
		else
			ep->DIEPCTL |= USB_OTG_DIEPCTL_SD0PID_SEVNFRM;
			
		ep->DIEPCTL |= (USB_OTG_DOEPCTL_CNAK | USB_OTG_DOEPCTL_EPENA);
	}
	else {
		USB_OTG_OUTEndpointTypeDef* ep = EPOUT(ep_num);
		
		//ep->DOEPCTL = (USB_OTG_DIEPCTL_EPDIS | USB_OTG_DIEPCTL_SNAK);
		
		//if(ep->DOEPCTL & USB_OTG_DIEPCTL_EONUM_DPID) {
		if((get_frame() & 1) == 0) {
			ep->DOEPCTL |= USB_OTG_DIEPCTL_SODDFRM;
			//ep->DOEPCTL &= ~USB_OTG_DIEPCTL_SODDFRM;
		}
		else {
			//ep->DOEPCTL &= ~USB_OTG_DIEPCTL_SD0PID_SEVNFRM;
			ep->DOEPCTL |= USB_OTG_DIEPCTL_SD0PID_SEVNFRM;
		}
