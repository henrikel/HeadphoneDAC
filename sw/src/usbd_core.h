...
//#define USB_PMA_SIZE        /**<\brief PMA memoty size in bytes. Adjust this for the devices that shares PMA memory with CAN in case of both USB and CAN in use to avoid data corruption. */
/** @} */
#endif
...
#define usbd_evt_incomplOUT	8	/* HE added */
#define usbd_evt_incomplIN	9	/* HE added */
#define usbd_evt_count      10	/* HE modified from 8 to 10 */
/** @}*/
...
typedef void (*usbd_hw_ep_activate)(uint8_t ep);
...
typedef void (*usbd_hw_flush_tx)(uint8_t ep);

typedef void (*usbd_hw_flush_rx)(void);

typedef void (*usbd_hw_toggle_sof)(uint8_t ep_num);
...
struct usbd_driver {
    ...
    usbd_hw_ep_activate		ep_activate;
    ...
    usbd_hw_flush_tx		flush_tx;
    usbd_hw_flush_rx		flush_rx;
    usbd_hw_toggle_sof		toggle_sof;
 ...
 * \copydetails usbd_hw_ep_deconfig
 */
inline static void usbd_ep_activate(usbd_device *dev, uint8_t ep) {
	dev->driver->ep_activate(ep);
}
...
inline static uint32_t usbd_getframe(usbd_device *dev) {
    return dev->driver->frame_no();
}

inline static void usbd_flush_tx(usbd_device *dev, uint8_t ep) {
	dev->driver->flush_tx(ep);
}

inline static void usbd_flush_rx(usbd_device *dev) {
	dev->driver->flush_rx();
}

inline static void usbd_toggle_sof(usbd_device *dev, uint8_t ep_num) {
	dev->driver->toggle_sof(ep_num);
}
